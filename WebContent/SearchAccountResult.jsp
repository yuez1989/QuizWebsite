...
<div class="body-section">
		...
		<div class='col-md-8 body-part-wrapper'>
			...
			<div class="column-name">SEARCH ACCOUNT RESULTS</div>
			
			<%
				String inputaccount = request.getParameter("searched_account");
				ArrayList<String> list = Utilities.searchAccounts(inputaccount);
				if(list.size() == 0){
					out.print("<span>None...</span>");
				}else{
					for(String str:list){
			%>
			
				<span class = 'news-feed'><a href='Person.jsp?person=<%=str %>'><%=str %></a></span>
			...
			<a href = 'AdminHomePage.jsp'>RETURN AMDIN PAGE</a>
			</div>
			</div>
		</div>
		<div class='col-md-2'></div>
